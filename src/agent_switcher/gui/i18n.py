...
_FA = {
    ...
    "IP Guard checks the public route before sign-in, usage checks, and account switching. Only a local fingerprint is saved; the raw IP is not stored.": "محافظ IP پیش از ورود، بررسی سهمیه و جابه‌جایی حساب مسیر عمومی را بررسی می‌کند. فقط یک اثر انگشت محلی ذخیره می‌شود و IP خام نگهداری نمی‌شود.",
    ...
    "IP Guard stopped {purpose} for {profile} before any OpenAI request was sent.\n\nPrevious fingerprint: {previous}\nCurrent fingerprint: {current}\n\nContinue only if you expected this network or proxy change.": "محافظ IP پیش از ارسال هرگونه درخواست به OpenAI، {purpose} برای {profile} را متوقف کرد.\n\nاثر انگشت قبلی: {previous}\nاثر انگشت فعلی: {current}\n\nفقط در صورتی ادامه دهید که انتظار این تغییر شبکه یا پروکسی را داشتید.",
